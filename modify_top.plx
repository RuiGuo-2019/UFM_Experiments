...
open(fh2,">", "INTERMEDIATEFOLDERPATH/plx_top_obf.v") or  die "can not open file";
$keysizeL = 0;
while(<fh1>) {
  if (/module\s+top\((.*)\)/) {
...
while(<fh3>) {
  ...
  } elsif(/sub_circuit\/iter$iter\/top.v/) { print fh4 "INTERMEDIATEFOLDERPATH/plx_top_obf.v\n";}
  else { print fh4 $_;}
}
close fh3;
close fh4;
