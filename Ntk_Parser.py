...
def ntk_parser(ipt_file):
        ...
        if line != "":
            if re.match(r'^#', line):  # Comment line
                ...
                if line_syntax:
                    ...
                    # line_syntax = re.match(r' *([a-zA-Z0-9_\S\$\[\]]+) *= *([a-zA-Z0-9_\S\[\]]+) *\( *(.+) *\)', line)
                    line_syntax = re.match(r' *([a-zA-Z0-9_\S\$\[\]]+) *= *', line)
        ...
        if line != "":
            if re.match(r'^#', line):
                ...
                if line_syntax:
                    ...
                    elif re.match(r'DFFRSE', line_syntax.group(2), re.IGNORECASE):
                        Circuit_graph.find_node_by_name(left_node).gate_type = Circuit_graph.gateType['DFFRSE']
                    ...
                    elif re.match(r'SDFF', line_syntax.group(2), re.IGNORECASE):
                        Circuit_graph.find_node_by_name(left_node).gate_type = Circuit_graph.gateType['DFF']
                        Circuit_graph.PPI.append(Circuit_graph.find_node_by_name(left_node))
                    ...
                    elif re.match(r'LUT', line_syntax.group(2), re.IGNORECASE):
                        Circuit_graph.find_node_by_name(left_node).gate_type = Circuit_graph.gateType['LUT']
                        Circuit_graph.find_node_by_name(left_node).value = int(line[line.find('LUT')+len('LUT'):line.find('(')], 16)
                    else:
                    ...
                    if(Circuit_graph.find_node_by_name(left_node).gate_type == Circuit_graph.gateType['DFFRSE']):
                        Circuit_graph.find_node_by_name(left_node).value = list([] if Circuit_graph.find_node_by_name(left_node).value is None else Circuit_graph.find_node_by_name(left_node).value)
                        for node in right_nodes:
                            ...
                            if("'b" not in node):
                                ...
                                Circuit_graph.find_node_by_name(left_node).value.append(node)
                            else:
                                Circuit_graph.find_node_by_name(left_node).value.append(node)
                    else:
                        for node in right_nodes:
    ...
    # check special node:
    # if(Circuit_graph.find_node_by_name('clock') is not None):
    if('clock' in Circuit_graph.name_to_node.keys()):
        node = Circuit_graph.find_node_by_name('clock')
        if((0 == len(node.fan_in_node)) and (0 == len(node.fan_out_node))):
            node.value = 0
        else:
            node.value = 1
...
def ntk_to_bench(Circuit_graph, opt_file_path):  # Write circuit_graph to .bench file
    ...
    if len(zipped):  # This a circuit has any PI
        ...
        for node in sorted_PI:
            if(('clock' == node.name) and (0 == node.value)):
                continue
    ...
    for node in Circuit_graph.object_list:
        if node.gate_type != Circuit_graph.gateType['IPT']:    
            if(node.gate_type == Circuit_graph.gateType['LUT']):
                opt_file.write("%s = %s %s (" % (node.name, Circuit_graph.gateType_reverse[node.gate_type], str(hex(node.value)))),
            elif(node.gate_type == Circuit_graph.gateType['DFFRSE']):
                strTemp = ""
                for signal in node.value:
                    strTemp = strTemp + signal + ', '
                if(', ' == strTemp[-2:]):
                    strTemp = strTemp[:-2]
                opt_file.write("%s = %s(%s) \n" % (node.name, Circuit_graph.gateType_reverse[node.gate_type], strTemp))
                continue
            else:
...
def seq_to_comb(input_path, output_path):
    ...
    for node in circuit_graph.object_list:  # TODO: Create a DFF list in the circuit_graph class to save time
        if node.gate_type == circuit_graph.gateType['DFF']:
            ...
            if(not isinstance(node.fan_in_node[0],str)):
                dff_ipt.append(node.fan_in_node[0].name)
            else:
                dff_ipt.append(node.fan_in_node[0])
