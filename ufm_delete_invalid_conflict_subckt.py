...
def delete_invalid_subckts(strRecordFile, strConflictFileRoot, strInvalidConflictRecordFileRoot):
    strConflictFile = os.path.join(strConflictFileRoot, strRecordFile)
    if(False == os.path.exists(strConflictFile)):
        print(strConflictFile,'not exist!')
        return
    strInvalidConflictRecordFile = os.path.join(strInvalidConflictRecordFileRoot, "InvalidConflictSubckt.txt")
    ...
    # back up old version
    strTime = time.strftime("%Y%m%d%H%M%S", time.localtime())
    strBackupFile = os.path.join(strConflictFileRoot, "backup_"+strTime+os.path.split(strConflictFile)[1])
    with open(strConflictFile, 'r') as cf:
        strConflictSubckt = cf.readline()
    with open(strBackupFile, 'w') as bauf:
            bauf.write(strConflictSubckt)
    ...
    print("Generate new " + strRecordFile + " finish!")
