...
class NtkObject:
    def __init__(self, name: str, category=0):
        ...
        self.category = category # Default sub-circuit
...
class Ntk:
    def __init__(self):
        self.gateType = {"IPT": 1, "NOT": 2, "NAND": 3, "AND": 4, "NOR": 5, "OR": 6, "XOR": 7, "XNOR": 8, "DFF": 9,
                         "BUFF": 10, "LUT": 11, "DFFRSE": 12}  # From string to integer
        self.gateType_reverse = {1: "IPT", 2: "NOT", 3: "NAND", 4: "AND", 5: "NOR", 6: "OR", 7: "XOR", 8: "XNOR",
                                 9: "DFF", 10: "BUFF", 11: "LUT", 12: "DFFRSE"}  # From integer to string
    ...
    def disconnect_objectives_from_circuit_graph(self, obj):
        for node in obj.fan_in_node:
            # obj.remove_fan_in(node)
            node.remove_fan_out(obj)
        for node in obj.fan_out_node:
            # obj.remove_fan_out(node)
            node.remove_fan_in(obj)
        obj.fan_in_node = []
        obj.fan_out_node = []
    ...
    def change_node_name(self, obj, new_name: str):
        del self.name_to_node[obj.name] # new
        self.object_name_list[self.object_list.index(obj)] = new_name
        obj.name = new_name
        self.name_to_node[obj.name] = obj # new
